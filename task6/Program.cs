...
class Program
{
    static void Main()
    {
        var pq = new MyPriorityQueue<int>(new int[] { 5, 3, 8, 1, 6 });
        Console.WriteLine("Размер: " + pq.Size());
        Console.WriteLine("Минимум: " + pq.Peek());

        pq.Add(0);
        Console.WriteLine("После добавления 0: " + pq.Peek());

        Console.WriteLine("Удалили: " + pq.Poll());
        Console.WriteLine("Теперь минимум: " + pq.Peek());

        pq.Remove(3);
        Console.WriteLine("После удаления 3: " + string.Join(", ", pq.ToArray()));

        pq.Clear();
        Console.WriteLine("Пуста ли очередь: " + pq.IsEmpty());
    }
}
